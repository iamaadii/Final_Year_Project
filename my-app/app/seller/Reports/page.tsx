...
export default function SellerReportsPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12 portal-page portal-module-transition">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-slate-900 flex items-center gap-3">
              <BarChart3 className="text-[#1b5b6a] w-8 h-8" /> {isPayables ? "Payables Reports" : "Financial Reports"}
            </h1>
            <p className="mt-2 text-sm text-slate-500 font-medium">{isPayables ? "Payables outflow, obligations, and AP aging insights." : "Profit & Loss, Balance Sheet, Cash Flow, and AR Aging reports."}</p>
          </div>
      ...
      <div className="flex bg-slate-100/80 p-1 rounded-2xl border border-slate-200/60 shadow-inner w-fit portal-section-enter portal-section-enter-delay-1">
        {TABS.map((tab) => (
          <button
            key={tab.key}
          ...
          >
            {tab.key === "aging" ? (isPayables ? "AP Aging" : "AR Aging") : tab.label}
          </button>
        ))}
      </div>
      ...
      <div className="rounded-3xl bg-white overflow-hidden portal-surface-soft portal-section-enter portal-section-enter-delay-1">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="rounded-2xl border border-emerald-100 bg-emerald-50/50 p-5">
                <p className="text-xs font-bold text-emerald-600 uppercase tracking-widest">{isPayables ? "Gross Outflow" : "Gross Revenue"}</p>
              ...
              <div className="rounded-2xl border border-[#cfe8e6] bg-[#e0f2f1]/50 p-5">
                <p className="text-xs font-bold text-[#1b5b6a] uppercase tracking-widest">{isPayables ? "Tax Impact" : "Tax Collected"}</p>
                <p className="mt-2 text-2xl font-black text-slate-900">{fmt(reportData.data.taxCollected)}</p>
              </div>
              <div className="rounded-2xl border border-amber-100 bg-amber-50/50 p-5">
                <p className="text-xs font-bold text-amber-600 uppercase tracking-widest">{isPayables ? "Early-Pay Discounts" : "Discounts"}</p>
                <p className="mt-2 text-2xl font-black text-slate-900">{fmt(reportData.data.discounts)}</p>
              </div>
              <div className="rounded-2xl border border-[#cfe8e6] bg-[#e0f2f1]/50 p-5">
                <p className="text-xs font-bold text-[#1b5b6a] uppercase tracking-widest">{isPayables ? "Net Outflow" : "Net Revenue"}</p>
          ...
          <div className="p-6 space-y-6">
            ...
            {(reportData.data.monthlyInflow.length > 0 || reportData.data.monthlyOutflow.length > 0) ? (
              <div className="h-[320px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={reportData.data.monthlyInflow.map((row) => ({
                      month: row.month,
                      inflow: row.total,
                      outflow: reportData.data.monthlyOutflow.find((x) => x.month === row.month)?.total || 0,
                    }))}
        ...
        {!loading && reportData.kind === "aging" && (
          <div className="p-6 space-y-6">
            <h2 className="text-xl font-black text-slate-800">{isPayables ? "AP Aging" : (reportData.report || "AR Aging")}</h2>
